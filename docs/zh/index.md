...
features:
  ...
  - icon: 🎯
    title: DNS 智能分流
    details: 基于 MosDNS 引擎的 DNS 分流，支持 FakeIP v4/v6 与 GeoIP 规则，精准识别国内外域名
  - icon: 🔄
    title: 双内核透明代理
...
主路由 (192.168.1.1)
    ...
    ↓ IPv6 路由: f2b0::/18 → fd00::2
    ↓
MSM 旁路由 (192.168.1.2)
    ├─ MosDNS (53端口) - DNS 分流
    │   ├─ 国内域名 → 国内 DNS
    │   └─ 国外域名 → FakeIP (28.0.0.0/8 / f2b0::/18)
    │
...
2. **FakeIP 模式**: 国外域名返回 FakeIP 地址（默认使用 `28.0.0.0/8` 与 `f2b0::/18`）
3. **静态路由**: 主路由将 FakeIP v4 / v6 流量路由到 MSM
...
- **FakeIP 模式**: 高效的双栈 FakeIP 实现，减少 DNS 泄漏
- **透明代理**: 无需客户端配置，全局透明代理
