...
> 当前稳定版本：`v0.9.12`  
> 发布时间：2026-03-04 13:27:13 CST  
> - 发布页：<https://github.com/msm9527/msm-wiki/releases/tag/0.9.12>  
> - 下载方式：同一发布页内提供各平台二进制与安装包

### ✨ 新增（Added）
- MiniSwitch 切换时显示旋转 loading 动画
- 订阅与代理节点支持注释开关并保留启用项
...
- 代理编辑改为暂存后统一保存并重启
- 订阅与代理节点合并“保存并重启”操作
- Proxies Tab 重构：状态与控件整合，Inline 展示
- 订阅与代理节点启用开关 UI 优化

### 🐛 修复（Fixed）
- 禁用代理节点添加状态标识及编辑修复
- 代理节点卡片隐藏服务器地址显示
- MiniSwitch 样式修复（颜色、禁用态适配）
- Geosite/广告拦截规则状态同步及兼容修复
- Mosdns 组件更新逻辑误判修复
- OpenRC 启动参数兼容性问题修复

### 📝 备注（Notes）
- Linux 服务默认配置目录固定为 /root/.msm
- 保存按钮简化为仅“保存并重启”，无需额外确认

::: details 📋 构建信息
- **发布通道**: stable（稳定版）
- **源提交**: [`a2e2819`](https://github.com/msm9527/msm/commit/a2e28191f56a85abaea6194388b336ededd078f2)
- **提交信息**: Merge pull request #26 from msm9527/dev
- **提交作者**: msm
- **提交时间**: 2026-03-04 13:27:13 CST
:::
