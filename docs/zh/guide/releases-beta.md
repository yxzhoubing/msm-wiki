...
### 🔧 变更（Changed）
- 性能提升：Mihomo 配置接口调用次数大幅减少
- 性能提升：SSE 数据迁移至 Zustand 消除重渲染
- 默认关闭运行时代理网络自愈功能
- 更新下载逻辑调整，跳过中转避免 Beta 版
- MosDNS 重试策略恢复原始配置

### 🐛 修复（Fixed）
- 修复更新重启死循环及构建残留备份
- 修复嵌入式前端启动黑屏问题
- 修复 Mihomo 版本显示异常及更新状态感知
- 修复强制更新后无法安装重启的竞态问题
- 修复 MosDNS 规则查询走系统代理及配置超时
- 修复轮播模式及外观设置页面的多处 Bug
- 修复日志页循环请求及窄屏布局显示问题
- 修复本地服务 HTTP 请求未绕过系统代理

### 📝 备注（Notes）
- 【严重】0.x 升级 1.x 必须重置安全并重新下载 DNS 服务
- 新旧版本 DNS 服务不兼容，升级前请务必备份配置
