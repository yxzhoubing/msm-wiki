...
- 调整运行时风险评分，降低二进制告警
- 默认关闭运行时代理网络自愈功能
- 合并 Mihomo 配置接口，减少 API 调用次数
- 迁移 SSE 数据至 Zustand，消除级联重渲染
- MosDNS 恢复原始重试策略

### 🐛 修复（Fixed）
- 修复更新重启死循环及构建备份残留
- 修复更新下载逻辑，防止 Beta 版本误装
- 修复嵌入式前端黑屏问题
- 修复进程页日志弹窗及配置刷新异常
- 修复本地服务及 MosDNS 系统代理相关问题
- 修复轮播模式 BUG 及外观设置页面
- 修复强制更新后无法安装重启及版本显示
- 修复日志页循环请求及路由上下文错误
- 修复 inline 规则集序列化及相关报错

### 📝 备注（Notes）
- ⚠️ 0.x 升 1.x 需重装或重置安全并重下 DNS 服务

::: details 📋 构建信息
- **发布通道**: beta（Beta 版）
- **源提交**: [`005d9cc`](https://github.com/msm9527/msm/commit/005d9ccb8ef800950267cde8e50a5c8bf7f2935a)
- **提交信息**: 调整运行时风险评分并降低二进制变更告警 / Adjust runtime risk scoring for binary-change alerts
- **提交作者**: root
- **提交时间**: 2026-04-20 15:51:41 CST
:::
